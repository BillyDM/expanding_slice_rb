...
impl<T: Default + Copy> ExpSliceRB<T> {
    ...
    /// buf.write(&data);
    ///
    /// let mut read_buf = [5u32; 4];
    /// let mut amount_written = buf.read_into(&mut read_buf);
    /// assert_eq!(amount_written, 4);
    /// assert_eq!(read_buf, [0u32, 1, 2, 0]);
    ///
    /// buf.write(&data);
    /// let mut large_read_buf = [5u32; 8];
    /// amount_written = buf.read_into(&mut large_read_buf);
    /// assert_eq!(amount_written, 5);
    /// assert_eq!(large_read_buf, [1u32, 2, 0, 1, 2, 5, 5, 5]);
    /// ```
    ...
    ///
    /// let mut read_buf = [5u32; 4];
    /// let mut amount_written = buf.peek_into(&mut read_buf);
    /// assert_eq!(amount_written, 4);
    /// assert_eq!(read_buf, [0u32, 1, 2, 0]);
    /// assert_eq!(buf.len(), 6);
    /// ```
